...
${text}`);
    ...
    startEditing() {
      ...
      if (window.getSelection) {
        ...
        if (range.isCollapsed) {
          range.selectAllChildren(this.dom.text);
          range.collapseToEnd();
        }
      }
  ...
  var VscodeExt = class {
    constructor() {
      this.vscode_api = typeof acquireVsCodeApi != "undefined" ? acquireVsCodeApi() : void 0;
    ...
    updateContent(text) {
      ...
      try {
        if (!text) {
          ...
          let format = repo6.get("native");
          let json2 = format.from(text);
          showMap(Map2.fromJSON(json2));
        }
      } catch {
        return;
      }
    ...
    updateTextDocument() {
      let format = repo6.get("native");
      var json = currentMap.toJSON();
      var dataContent = format === null || format === void 0 ? void 0 : format.to(json);
      this.vscode_api.postMessage({ type: "changed", data: dataContent });
    }
    delayUpdateTextDocument() {
      clearTimeout(this._timeout);
      this._timeout = setTimeout(this.updateTextDocument.bind(this), 500);
    }
    handleMessage(message, publisher, data) {
      switch (message) {
        case "item-change":
          if (this._initialized)
            this.delayUpdateTextDocument();
          break;
      }
    }
    init() {
      window.addEventListener("message", this.vscodeMessage.bind(this));
      subscribe("item-change", this);
      this.vscode_api.postMessage({ type: "ready" });
      document.querySelector("#ui").querySelector("p").style.display = "none";
      repo.get("save").execute = function() {
      };
      repo.get("save-as").execute = function() {
      };
      repo.get("load").execute = function() {
      };
      repo.get("new").execute = function() {
      };
    }
  };
  var vscExt = new VscodeExt();
  vscExt.init();
})();
